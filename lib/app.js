...
import { sign, verify } from './manifest.js';
import { createArchive, bundle } from './archive.js';

// The package root doubles as the library the loader (and any embedder)
// consumes: signing, verification, and archive creation.
export { sign, verify, createArchive, bundle };
...
create options:
  ...
  -c, --chain <file>    full certificate chain (PEM, leaf first); enables signing
      --comment <text>  archive comment (default: node:app)
...
async function create(args) {
    const { values } = parseArgs({
        ...
        options: {
            ...
            comment: { type: 'string', default: 'node:app' },
        },
    });
    ...
    console.error(key && chain
        ? `* signed manifest (${files.length} files)`
        : `* manifest (${files.length} files, unsigned)`);

    const out = values.output ? FS.createWriteStream(values.output) : process.stdout;
    await bundle({ base: values.base, files, prefix: values.prefix, key, chain, comment: values.comment, out });
    await end(out);
}
